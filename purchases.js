    ...
    if (applyToAll) {
      const updated = await pool.query(
        `UPDATE purchase_entries
         SET unit_price = $1, total_amount = quantity * $1 * (CASE WHEN is_return THEN -1 ELSE 1 END)
         WHERE product_id = $2
         RETURNING id`,
        [pricePerUnit, id]
      );
...
// OR (if isReturn is true) recording a return — same form, but the
// amount subtracts from the total instead of adding.
router.post("/entries", requireAuth, requireAdmin, async (req, res) => {
  const { productId, quantity, isReturn } = req.body;
  ...
  try {
    ...
    const totalAmount = unitPrice * Number(quantity) * (isReturn ? -1 : 1);

    const result = await pool.query(
      `INSERT INTO purchase_entries (product_id, product_name, quantity, unit_price, total_amount, is_return)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [productId, product.name, quantity, unitPrice, totalAmount, !!isReturn]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.error(err);
...
// product, or toggle return). Recalculates the total using the CURRENT
// price of whichever product ends up selected.
router.put("/entries/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE purchase_entries
       SET product_id = $1, product_name = $2, quantity = $3, unit_price = $4, total_amount = $5, is_return = $6
       WHERE id = $7 RETURNING *`,
      [productId, product.name, quantity, unitPrice, totalAmount, !!isReturn, id]
    );
