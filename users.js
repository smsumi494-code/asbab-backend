...
// PUT /api/users/me — any logged-in user updates their OWN name/profile
// picture (not an admin-only action — this is self-service).
router.put("/me", requireAuth, async (req, res) => {
  ...
  if (profilePictureUrl !== undefined) {
    sets.push(`profile_picture_url = $${values.length + 1}`);
  ...
  try {
    const result = await pool.query(
      `UPDATE users SET ${sets.join(", ")} WHERE id = $${values.length + 1}
       RETURNING id, phone, name, role, active, allowed_groups, profile_picture_url`,
      [...values, req.user.sub]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: "User not found" });
    const row = result.rows[0];
