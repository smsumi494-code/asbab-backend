...
async function updateParcelNote(pageId, email, password, consignmentId, newNote) {
  ...
  // The regular consignment/tracking page (already proven reliable via
  // getConsignmentTracking above) is the SAFEST source for the
  // customer's name/phone/address/COD amount — sending a wrong amount
  // specifically makes Steadfast reject the whole update as an
  // unauthorized "change the amount" attempt (their anti-fraud check),
  // even though we only meant to change the note.
  const trackRes = await fetch(`https://steadfast.com.bd/user/consignment/${consignmentId}`, {
    headers: { Cookie: cookie },
  });
  const trackHtml = await trackRes.text();
  if (trackHtml.includes("Moderator Login")) {
    sessionCache.delete(pageId);
    cookie = await getSessionCookie(pageId, email, password);
  }
  const known = parseConsignmentHtml(trackHtml);
  // codAmount on the tracking page is shown as "৳ 2230" — strip
  // anything that isn't a digit before sending it back as a number.
  const knownCodAmount = (known.codAmount || "").replace(/[^\d.]/g, "");
  ...
  // Less-critical fields still come from the edit form itself (field ids
  // are assumed to match the payload key names, e.g. id="item_description")
  // — getting one of THESE wrong doesn't trigger the same fraud block the
  // amount does, so it's lower-risk to leave as a best-effort read.
  const fieldVal = (id) => $(`#${id}`).val() || $(`#${id}`).attr("value") || "";

  const form = new FormData();
  form.append("consignment_id", String(consignmentId));
  form.append("cus_name", known.customerName || fieldVal("cus_name"));
  form.append("cus_phone", known.customerPhone || fieldVal("cus_phone"));
  form.append("cus_address", known.customerAddress || fieldVal("cus_address"));
  form.append("alt_phone", known.customerAltPhone || fieldVal("alt_phone"));
  ...
  form.append("cod_amount", knownCodAmount || fieldVal("cod_amount") || "0");
  ...
  // Steadfast can return HTTP 200 while still rejecting the update
  // internally (e.g. their anti-fraud "amount changed" check) — the
  // real result is inside the JSON body's "status"/"message" fields.
  try {
    const parsed = JSON.parse(resultText);
    if (parsed.status === 0 || parsed.status === false) {
      throw new Error(parsed.message || "Steadfast এই আপডেটটা প্রত্যাখ্যান করেছে");
    }
  } catch (parseErr) {
    // Not JSON, or already threw above — if it's our own thrown error,
    // let it propagate; otherwise the raw text wasn't parseable JSON,
    // which we treat as a soft success below (best-effort).
    if (parseErr.message && parseErr.message !== resultText && !(parseErr instanceof SyntaxError)) {
      throw parseErr;
    }
  }
