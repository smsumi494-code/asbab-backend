...
router.post("/woocommerce", async (req, res) => {
  ...
  try {
    ...
    // entry for this phone (same group, status='incomplete') so it
    // doesn't sit around as a stale duplicate.
    if (completedPhone) {
      await pool.query(
        "DELETE FROM entries WHERE group_name = 'website_order' AND status = 'incomplete' AND customer_phone = $1",
        [completedPhone]
      );
      broadcastRefresh();
    }
...
router.post("/woocommerce-incomplete", async (req, res) => {
  ...
  const rawText = [
    ...
    products ? `প্রোডাক্ট: ${products}` : null,
  ]
    .filter(Boolean)
    .join("\n");
  ...
  try {
    ...
    if (existing.rows.length) {
      await pool.query("UPDATE entries SET raw_text = $1 WHERE id = $2", [rawText, existing.rows[0].id]);
      broadcastRefresh();
    } else {
      await createEntry({
        rawText,
