...
const { router: entriesRouter, checkCourierStatusesAndSyncFacebook, sweepStaleWebsiteOrders, checkRiderNotes } = require("./entries");
...
async function scheduleCourierCheck() {
  ...
  try {
    const { sweepAbandonedOtps } = require("./public");
    await sweepAbandonedOtps();
  } catch (err) {
    console.error("Abandoned-OTP sweep failed:", err.message);
  }
  try {
    await sweepStaleWebsiteOrders();
  } catch (err) {
    console.error("Stale-order sweep failed:", err.message);
  }
  try {
    await checkRiderNotes();
  } catch (err) {
    console.error("Rider-note check failed:", err.message);
  }
  let minutes = 30;
  try {
