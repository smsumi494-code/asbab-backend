...
async function callGemini(userText, systemPrompt, apiKey, wantJson) {
  ...
  if (response.status === 429 || response.status === 503) {
    // 429 = our rate limit. 503 = Google's own servers overloaded — this
    // can persist longer than a quick blip, so retry more times with a
    // growing wait (3s, 6s, 9s, 12s) before giving up on this key.
    for (let i = 0; i < 4 && (response.status === 429 || response.status === 503); i++) {
