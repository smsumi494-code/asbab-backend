...
async function checkRiderNotes() {
  const result = await pool.query(
    `SELECT id, consignment_id, page_id, last_rider_note
     FROM entries
     WHERE group_name = 'all_order' AND consignment_id IS NOT NULL
       AND (courier_status IS NULL OR courier_status NOT IN ('delivered', 'partial_delivered', 'cancelled'))`
  );
...
router.get("/rider-notes", requireAuth, requireAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT ra.*, e.moderator, e.page_name, e.raw_text
...
router.post("/rider-notes/:id/seen", requireAuth, requireAdmin, async (req, res) => {
  try {
