...
async function callGemini(userText, systemPrompt, apiKey, wantJson) {
  ...
  if (response.status === 429 || response.status === 503) {
    // 429 = our rate limit (clears in seconds). 503 = Google's own
    // servers momentarily overloaded (also usually clears quickly). Two
    // short retries recover from most transient blips without
    // immediately burning through every fallback key.
    for (let i = 0; i < 2 && (response.status === 429 || response.status === 503); i++) {
