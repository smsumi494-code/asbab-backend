...
async function callGemini(userText, systemPrompt, apiKey, wantJson) {
  ...
  if (response.status === 429) {
    // Gemini's per-minute limit often clears within a few seconds — one
    // short wait-and-retry avoids immediately burning through every
    // fallback key just because of a momentary rate-limit blip.
...
const COMBINED_MAKING_SALES_PROMPT =
  ...
  "1) making_text — details a garment-making/production team needs, nothing about the customer or price. " +
  ...
  "STRICTLY EXCLUDE customer name, phone, address, and any bill/price/টাকা amount. If nothing relevant is " +
  "found, use an empty string.\n\n" +
  "2) cod_amount — the final bill/total (sometimes shown as a calculation like 2250+150=2400-500=1900 — use " +
  "the FINAL result; Bangla numerals count as normal digits).\n\n" +
  "3) garment_quantity — how many MAIN GARMENT pieces (borka/abaya) this order is for. Count ONLY the main " +
  "garment itself. Accessories like hijab (হিজাব), niqab (নিকাব), and inner (ইনার) do NOT add to the count — " +
  "e.g. '১ পিছ বোরকা হিজাবসহ', 'বোরকা + ইনার', 'বোরকা + নিকাব', 'বোরকা + হিজাব' are all garment_quantity 1. " +
  "But '২ পিছ বোরকা' or a plain '২ পিছ' clearly referring to garments is garment_quantity 2, and so on. " +
  ...
  "Respond with ONLY raw JSON, no markdown fences, no explanation, in exactly this shape: " +
  '{"making_text":"","cod_amount":0,"garment_quantity":1}.';

async function extractCombinedInfo(rawText, pageId) {
  return callAI(rawText, COMBINED_MAKING_SALES_PROMPT, { json: true, pageId });
}
...
router.post("/recalculate-sales", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    for (const row of result.rows) {
      try {
        const extracted = await extractCombinedInfo(row.raw_text, row.page_id);
