...
router.post("/:id/fraud-check", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // theirs. Always checked live (never cached) — it's rate-limited
    // per page/key, so caching it by phone alone would wrongly "stick"
    // an error from one page's key onto every other page's check for
    // the same phone.
    ...
    // 7-day cache by phone — for fraudbd.com only (Steadfast's official
    // check above is always fresh, see note there).
    const cached = await pool.query(
      "SELECT data FROM fraud_check_cache WHERE phone = $1 AND checked_at > NOW() - INTERVAL '7 days'",
      [phone]
    );
    if (cached.rows.length) {
      return res.json({ phone, cached: true, ownSummary, steadfastOfficial, ...cached.rows[0].data });
    }
    ...
    await pool.query(
      ...
      [phone, JSON.stringify(data.data)]
    );

    res.json({ phone, cached: false, ownSummary, steadfastOfficial, ...data.data });
  } catch (err) {
