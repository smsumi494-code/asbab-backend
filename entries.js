...
// customer's delivery history (Pathao/Steadfast/Paperfly/RedX) via
// fraudbd.com, using whichever phone number is on the entry (or found in
// its raw text if not yet extracted).
router.post("/:id/fraud-check", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({ phone, cached: false, ...data.data });
  } catch (err) {
