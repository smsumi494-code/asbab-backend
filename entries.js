...
async function createEntry({ rawText, imageUrls, moderator, group, pageId, pageName, status, customerPhone, customerDevice, salesDateChoice }) {
  ...
  if (targetGroup === "all_order") {
    ...
    // Sales Summary needs a bill AND a garment-piece-count estimate right
    // away too — extracted independently of Making, and completely
    // independent of Send to Courier. Whatever this AI extraction finds
    // NOW is kept forever, even if the order is later shipped with a
    // slightly different final bill — Sales Summary intentionally has no
    // relationship to courier data at all.
    (async () => {
      try {
        const extracted = await extractSalesInfo(rawText, pageId);
        ...
        console.error("Background sales-info extraction failed:", err);
      }
    })();
  }
