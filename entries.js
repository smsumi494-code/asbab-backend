...
router.post("/:id/fraud-check", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Fraud-check using — entry.page_id:", entry.page_id,
        "| courierCred found:", !!courierCred,
        "| key ends with:", sfKey ? sfKey.slice(-6) : "(none)"
      ); // temporary debug
      if (sfKey && sfSecret) {
        ...
        console.log("Steadfast fraud_check raw response:", JSON.stringify(sfData)); // temporary debug
        if (sfData?.message) {
