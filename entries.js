...
async function createEntry({ rawText, imageUrls, moderator, group, pageId, pageName, status, customerPhone, customerDevice, createdAt }) {
  const targetGroup = group || "pending";
  const batchId = randomUUID();
  const images = JSON.stringify(imageUrls || []);

  const result = await pool.query(
    `INSERT INTO entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, status, customer_phone, customer_device, created_at)
     VALUES ($1, $2::jsonb, $3, $4, $5, $6, $7, COALESCE($8, 'pending'), $9, $10, COALESCE($11::timestamp, NOW()))
     RETURNING *`,
    [rawText, images, moderator, targetGroup, batchId, pageId || null, pageName || null, status || null, customerPhone || null, customerDevice || null, createdAt || null]
  );

  if (targetGroup === "all_order") {
    await pool.query(
      `INSERT INTO entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, created_at)
       VALUES ($1, $2::jsonb, $3, 'pending', $4, $5, $6, COALESCE($7::timestamp, NOW()))`,
      [rawText, images, moderator, batchId, pageId || null, pageName || null, createdAt || null]
    );

    // Making group needs an AI call (1-3s) to pull out size/quantity —
    // running this in the background instead of awaiting it here means
    // the post itself confirms instantly. The Making card pops in a
    // moment later via the normal live SSE refresh.
    (async () => {
      try {
        let makingText;
        try {
        ...
        if (makingText) {
          await pool.query(
            `INSERT INTO entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, created_at)
             VALUES ($1, $2::jsonb, $3, 'making', $4, $5, $6, COALESCE($7::timestamp, NOW()))`,
            [makingText, images, moderator, batchId, pageId || null, pageName || null, createdAt || null]
          );
          broadcastRefresh();
        }
      } catch (err) {
        console.error("Background Making-group creation failed:", err);
      }
    })();

    // Sales Summary needs a bill estimate right away too — extracted
    // independently of Making, and completely independent of Send to
    // Courier. Whatever this AI extraction finds NOW is kept forever,
    // even if the order is later shipped with a slightly different final
    // bill — Sales Summary intentionally has no relationship to courier
    // data at all.
    (async () => {
      try {
        const extracted = await extractOrderInfo(rawText, pageId);
        const estimatedAmount = Number(extracted.cod_amount) || 0;
        await pool.query("UPDATE entries SET estimated_amount = $1 WHERE id = $2", [
          estimatedAmount,
          result.rows[0].id,
        ]);
      } catch (err) {
        console.error("Background estimated-amount extraction failed:", err);
      }
    })();
  }
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    // "গতকালের" day-choice — shifts created_at back exactly one day so
    // Sales Summary (and everything else) buckets this order under
    // yesterday's date instead of today's.
    let createdAt = null;
    if (req.body.dayChoice === "yesterday") {
      const d = new Date();
      d.setDate(d.getDate() - 1);
      createdAt = d.toISOString();
    }

    const entry = await createEntry({ ...req.body, rawText, pageName, createdAt });
