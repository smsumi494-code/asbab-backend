      ...
      if (entry.status !== "sent") {
        await pool.query(
          `INSERT INTO recycle_bin (original_group, raw_text, image_urls, moderator, page_id, page_name, status, customer_phone)
           VALUES ('all_order', $1, $2, $3, $4, $5, $6, $7)`,
          [entry.raw_text, entry.image_urls, entry.moderator, entry.page_id, entry.page_name, entry.status, entry.customer_phone]
        );
      }
    ...
    if (entry.group_name === "website_order" && entry.status !== "incomplete") {
      ...
      await pool.query(
        `INSERT INTO recycle_bin (original_group, raw_text, image_urls, moderator, page_id, page_name, delete_reason, status, customer_phone)
         VALUES ('website_order', $1, $2, $3, $4, $5, $6, $7, $8)`,
        [entry.raw_text, entry.image_urls, entry.moderator, entry.page_id, entry.page_name, reason, entry.status, entry.customer_phone]
      );
    }
