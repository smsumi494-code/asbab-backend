...
const { notifyAll, notifyUser } = require("./push");
const { addClient, removeClient, broadcastRefresh } = require("./sse");
const { getConsignmentTracking, updateParcelNote } = require("./steadfast_tracking");
...
function toApiShape(row) {
  return {
    ...
    exchangeRiderNote: row.exchange_rider_note,
  };
}
...
async function createEntry({ rawText, imageUrls, moderator, group, pageId, pageName, status, customerPhone, customerDevice, salesDateChoice, wooOrderId, fbp, fbc, exchangeReason, exchangeRiderNote }) {
  ...
  const result = await pool.query(
    `INSERT INTO entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, status, customer_phone, customer_device, sales_date, woo_order_id, fbp, fbc, exchange_reason, exchange_rider_note)
     VALUES ($1, $2::jsonb, $3, $4, $5, $6, $7, COALESCE($8, 'pending'), $9, $10,
       ...
       $12, $13, $14, $15, $16)
     RETURNING *`,
    [rawText, images, moderator, targetGroup, batchId, pageId || null, pageName || null, status || null, customerPhone || null, customerDevice || null, isYesterday, wooOrderId || null, fbp || null, fbc || null, targetGroup === "exchange" ? (exchangeReason || null) : null, targetGroup === "exchange" ? (exchangeRiderNote || null) : null]
  );
...
// POST /api/entries/:id/rider-note-reply — sends a NEW note back to
// Steadfast/the rider for this parcel (e.g. "আচ্ছা, আবার চেষ্টা
// করুন"). This is more fragile than reading tracking data — it depends
// on a CSRF token + XSRF cookie Steadfast issues, and possibly
// Cloudflare bot-protection, neither of which the read-only tracking
// scraper has to deal with.
router.post("/:id/rider-note-reply", requireAuth, async (req, res) => {
  const { id } = req.params;
  const note = (req.body?.note || "").trim();
  if (!note) return res.status(400).json({ error: "নোট লিখুন" });
  ...
  try {
    ...
    const result = await updateParcelNote(
      entry.page_id,
      modRes.rows[0].api_key,
      modRes.rows[0].secret_key,
      entry.consignment_id,
    ...
    console.error("Rider note reply failed:", err);
    res.status(500).json({ error: err.message || "নোট পাঠানো যায়নি" });
  }
});
...
router.get("/", requireAuth, async (req, res) => {
  try {
    // Without a limit, this grows forever as the business stays live —
    // after months of production use this was pulling down thousands of
    // rows (with images) on every single app open/refresh, which is the
    // most likely cause of the app feeling slow everywhere. Day-to-day
    // moderation never needs anything older than a few months; Sales
    // Summary/reports use their own separate, already-limited queries.
    //
    // all_order is excluded here — it's the biggest, fastest-growing
    // group by far, so it gets its own paginated endpoint
    // (/all-order-page) instead of being pulled in whole every time.
    const result = await pool.query(
      "SELECT * FROM entries WHERE created_at > NOW() - INTERVAL '90 days' AND group_name != 'all_order' ORDER BY created_at DESC"
    );
...
// GET /api/entries/counts — accurate per-group totals straight from the
// database (last 90 days, matching the main fetch's window), completely
// independent of how many All Order rows have actually been paginated
// into the frontend. Used for the Home screen's tile numbers.
router.get("/counts", requireAuth, async (req, res) => {
  try {
    ...
    const counts = {};
    result.rows.forEach((row) => {
      if (req.user.role === "admin" || !req.user.allowedGroups || req.user.allowedGroups.includes(row.group_name)) {
...
// GET /api/entries/all-order-page?offset=0&limit=300 — paginated All
// Order fetch. First page loads automatically on login (300), then the
// "See More" button asks for the next 200, and so on.
router.get("/all-order-page", requireAuth, async (req, res) => {
  if (req.user.role !== "admin" && req.user.allowedGroups && !req.user.allowedGroups.includes("all_order")) {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM entries
       WHERE created_at > NOW() - INTERVAL '90 days' AND group_name = 'all_order'
    ...
    res.json({ rows: rows.map(toApiShape), hasMore });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Could not load All Order page" });
  }
});
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    // All Exchange Order — a reason is mandatory, enforced here too (not
    // just in the UI), since this group exists specifically so every
    // exchange has a documented reason attached. The rider note (what's
    // going out vs what should come back) is mandatory too — it's what
    // actually gets sent to Steadfast as the courier note.
    if (targetGroup === "exchange" && !(req.body.exchangeReason || "").trim()) {
      return res.status(400).json({ error: "এক্সচেঞ্জের কারণ লিখতে হবে" });
    }
    if (targetGroup === "exchange" && !(req.body.exchangeRiderNote || "").trim()) {
      return res.status(400).json({ error: "রাইডারের জন্য নোট লিখতে হবে" });
    }
...
router.put("/:id", requireAuth, async (req, res) => {
  ...
  // Moderators can now edit All Order posts too, but their edit doesn't
  // apply right away — it waits in pending_edits for an Admin to approve
  // or decline from the Notification screen. Admins editing (any group)
  ...
  if (req.user.role !== "admin") {
    try {
      ...
      if (entry.group_name !== "all_order") {
        return res.status(403).json({ error: "শুধু All Order-এর পোস্ট এডিট করা যাবে" });
      }

      await pool.query(
        `INSERT INTO pending_edits (entry_id, submitted_by, proposed_raw_text, proposed_image_urls, original_raw_text, original_image_urls)
         VALUES ($1, $2, $3, $4::jsonb, $5, $6::jsonb)
         ON CONFLICT (entry_id, submitted_by) DO UPDATE SET
           proposed_raw_text = $3, proposed_image_urls = $4::jsonb, submitted_at = NOW()`,
        [
          id,
          req.body.moderator || req.user.name || req.user.phone,
          req.body.rawText ?? entry.raw_text,
          JSON.stringify(req.body.imageUrls ?? entry.image_urls ?? []),
          entry.raw_text,
          JSON.stringify(entry.image_urls || []),
        ]
      );
      broadcastRefresh();
      return res.json({ pendingApproval: true, message: "Admin অনুমোদনের অপেক্ষায় আছে" });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ error: "Could not submit edit for approval" });
    }
  }
...
// GET /api/entries/pending-edits — Admin only. Lists all moderator edits
// awaiting approval, with enough entry context to show a proper diff.
router.get("/pending-edits", requireAuth, requireAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT pe.*, e.page_name, e.moderator AS entry_moderator, e.batch_id
    ...
    res.json(
      result.rows.map((r) => ({
        ...
        proposedRawText: r.proposed_raw_text,
        proposedImageUrls: r.proposed_image_urls || [],
        originalRawText: r.original_raw_text,
        originalImageUrls: r.original_image_urls || [],
        submittedAt: r.submitted_at,
        pageName: r.page_name,
        entryModerator: r.entry_moderator,
      }))
    );
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Could not load pending edits" });
  }
});

// POST /api/entries/pending-edits/:id/approve — Admin only. Applies the
// proposed changes to the real entry, mirroring to Pending/Making the
// same way a direct Admin edit already does, then clears the request.
router.post("/pending-edits/:id/approve", requireAuth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    const pendingRes = await pool.query("SELECT * FROM pending_edits WHERE id = $1", [id]);
    if (pendingRes.rows.length === 0) {
      return res.status(404).json({ error: "পাওয়া যায়নি (হয়তো ইতিমধ্যে সিদ্ধান্ত নেওয়া হয়েছে)" });
    }
    const pending = pendingRes.rows[0];

    const result = await pool.query(
      `UPDATE entries SET raw_text = $1, image_urls = $2::jsonb WHERE id = $3 RETURNING *`,
      [pending.proposed_raw_text, JSON.stringify(pending.proposed_image_urls || []), pending.entry_id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Entry not found" });
    }
    ...
    // Mirror to Pending immediately, same as a direct Admin edit.
    if (updated.batch_id) {
      await pool.query(
        `UPDATE entries SET raw_text = $1, image_urls = $2::jsonb WHERE batch_id = $3 AND group_name = 'pending'`,
        [pending.proposed_raw_text, JSON.stringify(pending.proposed_image_urls || []), updated.batch_id]
      );

      // Making's re-sync needs an AI call — do it in the background so
      // approving doesn't sit there waiting on it.
      (async () => {
        try {
          let makingText;
          try {
            makingText = await extractMakingInfo(pending.proposed_raw_text, updated.page_id);
          } catch (err) {
            console.error("Making-group AI extraction failed, using fallback:", err);
            makingText = buildMakingText(pending.proposed_raw_text);
          }
          await pool.query(
            `UPDATE entries SET raw_text = $1, image_urls = $2::jsonb WHERE batch_id = $3 AND group_name = 'making'`,
            [makingText, JSON.stringify(pending.proposed_image_urls || []), updated.batch_id]
          );
          broadcastRefresh();
        } catch (err) {
          console.error("Background Making-group sync failed (approved edit):", err);
        }
      })();
    }

    await pool.query("DELETE FROM pending_edits WHERE id = $1", [id]);
    broadcastRefresh();
    res.json({ approved: true, entry: toApiShape(updated) });
  } catch (err) {
    console.error(err);
...
// POST /api/entries/pending-edits/:id/decline — Admin only. Discards the
// proposed edit — the real entry is never touched.
router.post("/pending-edits/:id/decline", requireAuth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("DELETE FROM pending_edits WHERE id = $1 RETURNING id", [id]);
    if (result.rows.length === 0) {
    ...
    console.error(err);
    res.status(500).json({ error: "Decline করা যায়নি" });
  }
});

// GET /api/entries/pending-deletes — Admin only. Lists moderator delete
// requests awaiting approval.
router.get("/pending-deletes", requireAuth, requireAdmin, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT pd.id, pd.entry_id, pd.submitted_by, pd.submitted_at, e.raw_text, e.image_urls, e.page_name, e.moderator AS entry_moderator
       FROM pending_deletes pd
       JOIN entries e ON e.id = pd.entry_id
    ...
    console.error(err);
    res.status(500).json({ error: "Could not load pending deletes" });
  }
});

// POST /api/entries/pending-deletes/:id/approve — Admin only. Actually
// deletes the entry now (and its Pending/Making/Emergency mirrors, same
// as a direct Admin delete), then clears the request. Recycles it first
// (same as a direct delete) unless it's already been sent to courier.
router.post("/pending-deletes/:id/approve", requireAuth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    const pendingRes = await pool.query("SELECT * FROM pending_deletes WHERE id = $1", [id]);
    if (pendingRes.rows.length === 0) {
      return res.status(404).json({ error: "পাওয়া যায়নি (হয়তো ইতিমধ্যে সিদ্ধান্ত নেওয়া হয়েছে)" });
    }
    const entryRes = await pool.query("SELECT * FROM entries WHERE id = $1", [pendingRes.rows[0].entry_id]);
    if (entryRes.rows.length === 0) {
      await pool.query("DELETE FROM pending_deletes WHERE id = $1", [id]);
      return res.status(404).json({ error: "মূল পোস্টটা আর নেই" });
    }
    ...
    if (entry.batch_id) {
      await pool.query(
        "DELETE FROM entries WHERE batch_id = $1 AND group_name IN ('pending', 'making', 'emergency')",
        [entry.batch_id]
      );
    }
...
// POST /api/entries/pending-deletes/:id/decline — Admin only. Discards
// the request — the real entry is never touched.
router.post("/pending-deletes/:id/decline", requireAuth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("DELETE FROM pending_deletes WHERE id = $1 RETURNING id", [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "পাওয়া যায়নি" });
    }
    broadcastRefresh();
...
router.delete("/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Moderators deleting an All Order post doesn't delete it right away
    // — it waits in pending_deletes for an Admin to approve/decline from
    // the Notification screen, same pattern as the edit-approval flow.
    // Admins deleting (any group) still applies immediately, same as before.
    if (req.user.role !== "admin") {
      if (entry.group_name !== "all_order" && entry.group_name !== "exchange") {
        return res.status(403).json({ error: "শুধু All Order/Exchange-এর পোস্ট ডিলিট করার অনুরোধ পাঠানো যাবে" });
      }
      await pool.query(
        `INSERT INTO pending_deletes (entry_id, submitted_by)
         VALUES ($1, $2)
         ON CONFLICT (entry_id) DO UPDATE SET submitted_by = $2, submitted_at = NOW()`,
        [id, req.body?.moderator || req.user.name || req.user.phone]
      );
      broadcastRefresh();
      return res.json({ pendingApproval: true, message: "Admin অনুমোদনের অপেক্ষায় আছে" });
    }
    ...
    // All Exchange Order — before it's sent to courier, a password is
    // required (still an easy-to-undo mistake at that point). Once it's
    // been sent, no password needed — matches how All Order treats a
    // sent parcel too (nothing left to accidentally still ship).
    if (entry.group_name === "exchange" && entry.status !== "sent") {
      const expected = process.env.ALL_ORDER_DELETE_PASSWORD || "Asbab";
      if (req.body?.password !== expected) {
        return res.status(403).json({ error: "ভুল পাসওয়ার্ড" });
      }
    ...
    if (entry.group_name === "website_order" && entry.status !== "incomplete") {
      ...
      if (entry.woo_order_id) {
        (async () => {
          try {
            await sendFacebookEvent(entry.page_id, entry.customer_phone, "OrderRefunded", entry.customer_name, entry.fbp, entry.fbc);
    ...
    if (entry.group_name === "pending" && entry.batch_id) {
      await pool.query(
        "DELETE FROM entries WHERE batch_id = $1 AND group_name IN ('making', 'emergency')",
        [entry.batch_id]
      );
    }

    // Emergency is a mirror of Pending — deleting one removes the other,
    // AND the linked Making copy too (this was previously missed here,
    // which let Making silently accumulate orphaned entries over time
    // whenever a delete happened from Emergency instead of Pending).
    if (entry.group_name === "emergency" && entry.batch_id) {
      await pool.query(
...
// POST /api/entries/:id/send-to-emergency — Admin only. Copies this
// entry (from All Order or Pending) into the Emergency group. The
// original stays exactly where it was — this is a COPY, not a move.
// Refuses to create a second copy for the same batch_id.
router.post("/:id/send-to-emergency", requireAuth, async (req, res) => {
  const { id } = req.params;
  try {
    const existing = await pool.query("SELECT * FROM entries WHERE id = $1", [id]);
    if (existing.rows.length === 0) {
    ...
    if (entry.batch_id) {
      const already = await pool.query(
        "SELECT id FROM entries WHERE batch_id = $1 AND group_name = 'emergency'",
        [entry.batch_id]
      );
      if (already.rows.length > 0) {
        return res.status(400).json({ error: "ইতিমধ্যে Emergency-তে পাঠানো হয়েছে" });
      }
    }
    ...
    // Force the new copy onto the SAME batch as the source entry, so the
    // Pending<->Emergency sync logic (delete/send-to-courier mirroring)
    // recognizes them as linked. createEntry always makes a fresh
    // batch_id, so overwrite it here.
    if (entry.batch_id) {
      await pool.query("UPDATE entries SET batch_id = $1 WHERE id = $2", [entry.batch_id, newEntry.id]);
    }
    ...
    console.error(err);
    res.status(500).json({ error: "Could not send to Emergency" });
  }
});
...
// POST /api/entries/fraud-check-phone — Admin only. Same as the
// per-entry fraud-check below, but for a manually-typed phone number —
// used by the standalone "Fraud Check" button (Pending group's bottom
// nav), so a moderator can look up any customer before an order even
// exists yet. Defaults to Al-Haya's Steadfast key (same fallback pattern
// used elsewhere for "no specific page context" lookups).
router.post("/fraud-check-phone", requireAuth, async (req, res) => {
  let phone = normalizeBDPhone(req.body?.phone);
  if (!phone || phone.length !== 11) {
    return res.status(400).json({ error: "সঠিক ফোন নাম্বার দিন" });
  }
  try {
    ...
    try {
      ...
      const courierCred = alHayaPage.rows.length
    ...
    console.error(err);
    res.status(500).json({ error: "চেক করা যায়নি" });
  }
});
...
router.post("/:id/send-to-courier", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // the delivery rider sees it too. All Exchange Order overrides this
    // with the moderator's own per-order rider note (written at post
    // time — what's going out vs what should come back) — this is far
    // more useful to the rider than one fixed generic instruction, since
    // it's specific to what's actually in THIS parcel. Every other group
    // keeps using the normal per-page note, unchanged.
    let deliveryNote = null;
    if (entry.group_name === "exchange") {
      deliveryNote = entry.exchange_rider_note || null;
    } else if (entry.page_id) {
    ...
    // Safety cap — Steadfast appears to error out (returns an HTML error
    // page instead of JSON) on notes that are too long, so truncate
    // defensively regardless of where the note came from.
    if (deliveryNote && deliveryNote.length > 150) {
      deliveryNote = deliveryNote.slice(0, 150);
    }
    ...
    // stuck on "waiting for courier send". Pending and Emergency both
    // feed into this the same way.
    if ((entry.group_name === "pending" || entry.group_name === "emergency") && entry.batch_id) {
      await pool.query(
        `UPDATE entries
         SET status = 'sent',
             consignment_id = $1,
      ...
      // Pending and Emergency are "mirrors" of each other — whichever one
      // this Send to Courier happened from, remove the copy sitting in
      // the OTHER one, so it doesn't linger there un-shipped. The linked
      // Making entry is done its job too once the parcel actually ships
      // — remove it here as well (previously missed, which let Making
      // silently accumulate more entries than Pending over time).
      const siblingGroup = entry.group_name === "pending" ? "emergency" : "pending";
      await pool.query(
        "DELETE FROM entries WHERE batch_id = $1 AND group_name IN ($2, 'making')",
        [entry.batch_id, siblingGroup]
      );
    }
...
async function checkCourierStatusesAndSyncFacebook() {
  // Covers EVERY All Order entry that's been sent to courier and doesn't
  // have a final status yet — not just website orders. Manually-posted
  // orders (the majority) need their courier_status kept current too,
  // since Sales Summary, Fraud Check, System Alerts, and the AI
  // assistant all rely on it. Facebook/WooCommerce syncing below still
  // only applies when there's an actual website order to sync back to.
  const result = await pool.query(
    `SELECT id, consignment_id, page_id, customer_phone, customer_name, woo_order_id, fbp, fbc, courier_status AS previous_status
     FROM entries
     WHERE group_name = 'all_order' AND consignment_id IS NOT NULL
       AND (courier_status IS NULL OR courier_status NOT IN ('delivered', 'partial_delivered', 'cancelled'))`
  );
  ...
  for (const entry of result.rows) {
    try {
      ...
      // Always keep courier_status current, regardless of whether this
      // is a website order — this alone drives Sales Summary/Fraud
      // Check/System Alerts/AI assistant figures. courier_status_updated_at
      // records WHEN it CHANGED to this status, which is what "কয়টা
      // ডেলিভারি হয়েছে আজকে" actually needs — NOT when the order was
      // posted. Only stamped on an actual change, so re-checking the same
      // still-pending status every 30 min doesn't keep resetting it.
      if (status !== entry.previous_status) {
        await pool.query(
          "UPDATE entries SET courier_status = $1, courier_status_updated_at = NOW() WHERE id = $2",
          [status, entry.id]
        );
      }

      // Facebook CAPI + WooCommerce sync only makes sense for orders
      // that actually came from the website AND haven't been synced yet.
      if (!entry.woo_order_id) continue;
      ...
      if (statusLower.includes("cancel")) {
        ...
        // Catches both the plain "delivered" and Steadfast's variants
        // like "delivered_approval_pending" (rider marked it delivered,
        // office hasn't confirmed cash collection yet — but the customer
        // DID receive the product, which is what matters here).
...
// Checks Steadfast's tracking timeline (via the scraper in
// steadfast_tracking.js) for every All Order parcel that's been sent to
// courier but isn't finalized yet, and alerts Admin the instant the
// RIDER adds a genuinely new note/status message — never repeats the
// same note twice for the same parcel. Uses the Moderator-panel login
// (email/password saved per page as type='steadfast_moderator'), which
// is a real browser-style login rather than an official API, so this is
// inherently a bit more fragile than the official status_by_cid check
// above — if Steadfast changes their page layout, this specific feature
// (not the rest of the app) would need updating.
async function checkRiderNotes() {
  const result = await pool.query(
    `SELECT id, consignment_id, page_id, last_rider_note, moderator, customer_name, customer_phone
     FROM entries
     WHERE group_name = 'all_order' AND consignment_id IS NOT NULL
       AND (courier_status IS NULL OR courier_status NOT IN ('delivered', 'partial_delivered', 'cancelled'))`
  );
  ...
  for (const entry of result.rows) {
    try {
      const modCred = await pool.query(
        "SELECT api_key, secret_key FROM api_credentials WHERE page_id = $1 AND type = 'steadfast_moderator' LIMIT 1",
        [entry.page_id]
      );
      if (modCred.rows.length === 0) continue;

      const tracking = await getConsignmentTracking(
        entry.page_id,
        modCred.rows[0].api_key,
        modCred.rows[0].secret_key,
        entry.consignment_id
      );
      const steps = tracking?.trackingSteps || [];
      if (steps.length === 0) continue;

      // Only genuine rider notes start with "Rider Note:" — everything
      // else in the timeline (e.g. "Consignment created by Sender(API)")
      // is a system-generated tracking event, not something the rider
      // actually wrote. The timeline lists newest-first, so the first
      // match here is the most recent rider note.
      const riderNoteSteps = steps.filter((s) => s.message && s.message.includes("Rider Note:"));
      if (riderNoteSteps.length === 0) continue;
      const latest = riderNoteSteps[0];

      if (!latest.message || latest.message === entry.last_rider_note) continue;

      const cleanNote = latest.message.replace(/^Rider Note:\s*/i, "").trim();

      // Only alert about a note from TODAY — older notes (from before
      // this feature existed, or ones that were already relevant on a
      // previous day) shouldn't suddenly show up as "new".
      const noteDay = latest.date ? new Date(latest.date) : null;
      const bdNow = new Date(Date.now() + 6 * 60 * 60 * 1000);
      const isToday = noteDay && !isNaN(noteDay) && noteDay.toDateString() === bdNow.toDateString();
      if (!isToday) {
        // Still remember it so we don't keep re-checking the same old
        // note forever, just don't alert about it.
        await pool.query("UPDATE entries SET last_rider_note = $1 WHERE id = $2", [latest.message, entry.id]);
        continue;
      }

      await pool.query(
        `INSERT INTO rider_note_alerts (entry_id, consignment_id, note_message, note_date, note_time)
         VALUES ($1, $2, $3, $4, $5)`,
        [entry.id, entry.consignment_id, cleanNote, latest.date || null, latest.time || null]
      );
      await pool.query("UPDATE entries SET last_rider_note = $1 WHERE id = $2", [latest.message, entry.id]);
      broadcastRefresh();

      // Alert the moderator who took this order via a push notification
      // (not WhatsApp — automating regular WhatsApp risks getting the
      // number banned; not SMS either — moderators don't routinely check
      // their SMS inbox). A push notification lands right on their phone's
      // lock screen/notification tray even with the app closed, the same
      // way a WhatsApp message would, but through our own app safely.
      try {
        if (entry.moderator) {
          const modUserRes = await pool.query("SELECT id FROM users WHERE name = $1 LIMIT 1", [entry.moderator]);
          const modUserId = modUserRes.rows[0]?.id;
          if (modUserId) {
            await notifyUser(modUserId, {
              title: "🛵 রাইডার নোট এসেছে",
              body: `${entry.customer_name || "কাস্টমার"} (${entry.customer_phone || ""}): ${cleanNote}`,
            });
          }
        }
      } catch (pushErr) {
        console.warn("Rider-note push alert failed for entry", entry.id, pushErr.message);
      }
    } catch (err) {
      // Login/scraping can genuinely fail sometimes (Steadfast down,
      // session hiccup, page layout changed) — log and move on instead
      // of letting one failing parcel stop everyone else's check.
      console.warn("Rider note check failed for entry", entry.id, err.message);
    }
  }
}

// GET /api/entries/rider-notes — any logged-in user. Every moderator and
// Admin sees the same unseen rider-note alerts, regardless of who
// posted that particular order (shared visibility, so anyone free at
// the moment can follow up).
router.get("/rider-notes", requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT ra.*, e.moderator, e.page_name, e.raw_text, e.customer_phone
    ...
    console.error(err);
    res.status(500).json({ error: "Could not load rider notes" });
  }
});

// POST /api/entries/rider-notes/:id/seen — Admin only. Dismisses one alert.
// POST /api/entries/rider-notes/:id/seen — any logged-in user (a
// moderator can dismiss a note on their own order; Admin can dismiss any).
// POST /api/entries/rider-notes/:id/seen — any logged-in user can
// dismiss any note (matches the shared/universal visibility above).
router.post("/rider-notes/:id/seen", requireAuth, async (req, res) => {
  try {
    await pool.query("UPDATE rider_note_alerts SET seen = true WHERE id = $1", [req.params.id]);
    res.json({ seen: true });
  } catch (err) {
    console.error(err);
...
// sat unresolved (never sent to All Order/courier, never formally
// deleted with a reason) for too long. Usually means the customer never
// answered the confirmation call and everyone simply moved on without
// closing it out — so Facebook never got told this "Purchase" didn't
// actually go anywhere. Treats it exactly like a cancel: Refund event +
// WC status update, then marks it so it's never reprocessed.
async function sweepStaleWebsiteOrders() {
  let staleDays = 3;
  try {
    const { getSetting } = require("./app_settings");
    staleDays = Number(await getSetting("stale_order_days")) || 3;
  } catch (err) {
    console.error("Could not read stale_order_days, using default:", err.message);
  }

  const result = await pool.query(
    `SELECT id, page_id, customer_phone, customer_name, woo_order_id, fbp, fbc
     FROM entries
     WHERE group_name = 'website_order'
       AND status != 'incomplete'
       AND woo_order_id IS NOT NULL
       AND fb_event_sent IS NULL
  ...
  for (const entry of result.rows) {
    try {
      ...
      if (!fbResult.success) console.warn("Stale-order Facebook event failed for entry", entry.id, fbResult.error);
      if (!wcResult.success) console.warn("Stale-order WC status update failed for entry", entry.id, wcResult.error);
      await pool.query("UPDATE entries SET fb_event_sent = 'refund' WHERE id = $1", [entry.id]);
    } catch (err) {
      console.error("Stale-order sweep failed for entry", entry.id, err.message);
    }
  }
}

module.exports = { router, createEntry, sendSMS, getPageSmsSettings, fillSmsTemplate, checkCourierStatusesAndSyncFacebook, sweepStaleWebsiteOrders, checkRiderNotes };
