...
router.post("/fraud-check-phone", requireAuth, requireAdmin, async (req, res) => {
...
// GET /api/entries/rider-notes — Admin only. Unseen rider-note alerts.
// GET /api/entries/rider-notes — any logged-in user. Admins see every
// unseen rider note; moderators only see notes on orders THEY posted
// (they get a push notification for these, so they need a way to
// revisit/re-read them in the app afterward).
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT ra.*, e.moderator, e.page_name, e.raw_text, e.customer_phone
       ...
       WHERE ra.seen = false ${isAdmin ? "" : "AND e.moderator = $1"}
       ORDER BY ra.created_at DESC`,
      isAdmin ? [] : [req.user.name]
    );
    res.json(
      result.rows.map((r) => ({
        id: r.id,
  ...
  try {
    if (req.user.role !== "admin") {
      const check = await pool.query(
        `SELECT e.moderator FROM rider_note_alerts ra JOIN entries e ON e.id = ra.entry_id WHERE ra.id = $1`,
        [req.params.id]
      );
      if (check.rows.length === 0) return res.status(404).json({ error: "পাওয়া যায়নি" });
      if (check.rows[0].moderator !== req.user.name) {
        return res.status(403).json({ error: "এটা আপনার অর্ডার না" });
      }
    }
