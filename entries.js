...
async function createEntry({ rawText, imageUrls, moderator, group, pageId, pageName, status, customerPhone, customerDevice, salesDateChoice, wooOrderId, fbp, fbc, exchangeReason }) {
  ...
  const result = await pool.query(
    `INSERT INTO entries (raw_text, image_urls, moderator, group_name, batch_id, page_id, page_name, status, customer_phone, customer_device, sales_date, woo_order_id, fbp, fbc, exchange_reason)
     VALUES ($1, $2::jsonb, $3, $4, $5, $6, $7, COALESCE($8, 'pending'), $9, $10,
       ...
       $12, $13, $14, $15)
     RETURNING *`,
    [rawText, images, moderator, targetGroup, batchId, pageId || null, pageName || null, status || null, customerPhone || null, customerDevice || null, isYesterday, wooOrderId || null, fbp || null, fbc || null, targetGroup === "exchange" ? (exchangeReason || null) : null]
  );
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    // exchange has a documented reason attached.
...
router.delete("/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (entry.group_name === "emergency" && entry.batch_id) {
      await pool.query(
        "DELETE FROM entries WHERE batch_id = $1 AND group_name = 'pending'",
        [entry.batch_id]
      );
    }
...
router.post("/:id/send-to-courier", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // with a fixed instruction specific to exchanges — every other group
    // keeps using the normal per-page note, unchanged.
    let deliveryNote = null;
    if (entry.group_name === "exchange") {
      deliveryNote =
        "এক্সচেঞ্জ পার্সেল — ডেলিভারি না, পার্শিয়াল দেখাবেন। পার্সেল খুলে দেখিয়ে বদল করে দিন, প্রয়োজনে মার্চেন্টকে কল দিন।";
    ...
    if ((entry.group_name === "pending" || entry.group_name === "emergency") && entry.batch_id) {
      ...
      // the OTHER one, so it doesn't linger there un-shipped.
      const siblingGroup = entry.group_name === "pending" ? "emergency" : "pending";
      await pool.query("DELETE FROM entries WHERE batch_id = $1 AND group_name = $2", [entry.batch_id, siblingGroup]);
    }
