...
// GET /api/chat/latest — lightweight, used just to check whether there
// are new messages since the last time this device looked (for the
// bottom-nav badge), without pulling all 100 messages down every time.
router.get("/latest", requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, created_at FROM chat_messages ORDER BY id DESC LIMIT 1"
    );
    if (result.rows.length === 0) return res.json({ latestId: 0, latestAt: null });
    res.json({ latestId: result.rows[0].id, latestAt: result.rows[0].created_at });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Could not check latest message" });
  }
});
