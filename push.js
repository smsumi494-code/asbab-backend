...
// Sends a notification to just ONE specific user's device(s) — used for
// alerts that only matter to a particular moderator (e.g. a new rider
// note on an order THEY posted), instead of broadcasting to everyone.
        ...
        webpush
          ...
          .catch(async (err) => {
            if (err.statusCode === 410 || err.statusCode === 404) {
              ...
              console.error("Targeted push send failed:", err.message);
            }
          })
      )
    );
...
module.exports = { router, notifyAll, notifyUser };
