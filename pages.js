...
router.post("/", requireAuth, requireAdmin, async (req, res) => {
  const { name, tagline, courierApiKey, courierSecretKey, aiCredentials } = req.body;
  ...
  try {
    ...
    const pageResult = await client.query(
      "INSERT INTO pages (name, tagline) VALUES ($1, $2) RETURNING *",
      [name, tagline || null]
    );
...
router.put("/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const pageResult = await client.query(
      "UPDATE pages SET name = $1, tagline = $2 WHERE id = $3 RETURNING *",
      [name, tagline || null, id]
    );
