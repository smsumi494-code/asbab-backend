  ...
  try {
    ...
    if (moderatorEmail && moderatorPassword) {
      const existingMod = await client.query(
        "SELECT id FROM api_credentials WHERE page_id = $1 AND type = 'steadfast_moderator'",
        [id]
      );
      if (existingMod.rows.length) {
        await client.query(
          "UPDATE api_credentials SET api_key = $1, secret_key = $2 WHERE id = $3",
          [moderatorEmail, moderatorPassword, existingMod.rows[0].id]
        );
      } else {
        await client.query(
          `INSERT INTO api_credentials (type, provider, api_key, secret_key, page_id)
