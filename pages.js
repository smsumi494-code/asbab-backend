...
router.post("/", requireAuth, requireAdmin, async (req, res) => {
  const { name, courierApiKey, courierSecretKey, aiCredentials } = req.body;
  ...
  try {
    ...
    const pageResult = await client.query("INSERT INTO pages (name) VALUES ($1) RETURNING *", [name]);
...
router.put("/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const pageResult = await client.query(
      "UPDATE pages SET name = $1 WHERE id = $2 RETURNING *",
      [name, id]
    );
