...
{"startDate": "YYYY-MM-DD", "endDate": "YYYY-MM-DD"}`;
    ...
    const statusChangedRes = await pool.query(
      `SELECT moderator, page_name, estimated_amount, estimated_quantity, courier_status, sales_date
       FROM entries
       WHERE group_name = 'all_order'
         AND courier_status_updated_at >= $1::date AND courier_status_updated_at < ($2::date + INTERVAL '1 day')
         AND courier_status IN ('delivered', 'partial_delivered', 'cancelled')`,
    ...
    // ---- Rider notes — ALWAYS all currently-unseen ones, regardless of
    // the detected date range. Someone asking "রাইডার কি নোট দিয়েছে" or
    // "নোটগুলো দেখাও" wants to know what's currently pending attention,
    // not strictly bounded by whatever date the range-detection step
    // guessed — those notes may span several days and still be relevant.
    ...
    const statsPayload = {
      ...
      unseenRiderNotes: {
        note: "রাইডার (ডেলিভারিম্যান) যেসব নোট/মেসেজ দিয়েছে যা এখনো Admin দেখেননি (Notification স্ক্রিনে 'Read' চাপা হয়নি) — 'রাইডার কি নোট দিয়েছে', 'নোটগুলো দেখাও' জাতীয় যেকোনো প্রশ্নের জন্য এটাই ব্যবহার করুন, প্রশ্নের ভাষা যেভাবেই হোক না কেন",
...
না কেন) "unseenRiderNotes" অংশ থেকে উত্তর দিন — প্রতিটা নোট কোন অর্ডারের, কোন
মডারেটরের, এবং রাইডার ঠিক কী লিখেছে তা স্পষ্টভাবে জানান। কোনো নোট না থাকলে
সরাসরি বলুন কোনো নতুন নোট নেই।
