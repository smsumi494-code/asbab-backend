...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      name: user.name,
      phone: user.phone,
      allowedGroups: user.allowed_groups || null,
    });
  } catch (err) {
    console.error(err);
...
// Requires a valid token (any role). Looks up role/active/allowed_groups
// FRESH from the database on every request (instead of trusting the JWT's
// embedded values) so that permission changes and deactivation take
// effect immediately — not just after the person's next login, which
// could be up to 30 days away.
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const result = await pool.query("SELECT role, active, allowed_groups FROM users WHERE id = $1", [decoded.sub]);
    ...
    req.user = {
      ...decoded,
      role: dbUser.role,
      allowedGroups: dbUser.allowed_groups || null,
    };
    next();
  } catch (err) {
